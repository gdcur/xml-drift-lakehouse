...
[ LLM Field Mapping ]  ← only fires if drift detected
  ...
  - Vector store retrieval (true RAG) — planned as drop-in upgrade, not yet implemented
...
├── docker/
│   ├── docker-compose.yml         # Airflow + Superset + Postgres stack
│   ├── Dockerfile                 # Custom Airflow image with project deps
│   ├── Dockerfile.superset        # Custom Superset image with duckdb-engine
│   ├── .env.example               # Environment template — copy to .env
│   └── requirements.txt
├── scripts/
│   ├── verify.py                  # Pipeline health check + drift registry
│   ├── report.py                  # Business analytics summary (terminal)
│   └── load_to_snowflake.py       # Load Parquet files into Snowflake RAW schema
├── superset/
│   └── dashboard_export_*.zip     # Exportable Superset dashboard definition
├── docs/
│   ├── airflow_dag_run.png
│   ├── ai_stress_test.png
│   ├── rag_mapping_output.png
│   ├── snowflake_mart_invoices.png
│   └── superset_dashboard.png
├── tests/
│   ├── test_rag_flow.py            # 29 integration tests — no API key needed
│   └── test_parser.py             # Parser diagnostic tool
...
```bash
python scripts/verify.py --resolve DocumentNbr
```
